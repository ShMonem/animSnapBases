...
class posComponents:  # Components == bases
    def __init__(self):
        ...
        self.output_components_dir = vertPos_output_bases_dir
        self.output_animation_dir = vertPos_output_animation_dir
    ...
    def compute_support_map(idx, geodesics, min_dist, max_dist):  # In paper Eq(6) before multiplying with lambda
        phi = geodesics(idx)
        return (np.clip(phi, min_dist, max_dist) - min_dist) \
               / (max_dist - min_dist)
    ...
    def extract_k_components(self, residual_init, snapshots_compute_geodesic_distance, writer,
        ...
        R = residual_init
        # np.save("residual_init_componentClasses", R)
        C = []
        W = []
        s = None
        for k in range(self.numComp):
            ...
            # wk is (F,), R is (F, N, 3), np.tensordot(wk, R, (0, 0)) is (N, 3), s[:,np.newaxis] is (N, 1)
            # components are normalized by ||wk||, each component is (N,3)
            if self.support == 'local':
                ck = (np.tensordot(wk, R, (0, 0)) * s[:, np.newaxis]) \
                     / np.inner(wk, wk)
            else:
                ck = np.tensordot(wk, R, (0, 0)) / np.inner(wk, wk)
    ...
    def splocs_glob_optimization(self, num_iters_max, num_admm_iterations,
        ...
        for it in range(num_iters_max):
            ...
            E_rms = norm(R)/np.sqrt(3*self.pos_snapshots.nVerts*self.pos_snapshots.frs)   # Kavan et.al. 2010
    ...
    def test_convergence(self, start, end, step):
        ...
        # snapshots_inf_norm = np.argmax(np.linalg.norm(snapshots.reshape(e, p, -1), ord='fro', axis=(1, 2)))
        denom = np.sqrt(3*self.pos_snapshots.nVerts *self.pos_snapshots.frs)

        for k in range(start, end + 1, step):
            res = snapshots[:, :, 0] - np.tensordot(self.weigs[:, :k], self.comps[:k, :, 0], axes=([1, 0]))  # residual
    ...
    def post_process_components(self, standarize, massWeight, orthogonal):
        ...
        if standarize:
            ...
            self.comps += self.pos_snapshots.mean[np.newaxis]  # (K, N, 3) + (1, N, 3)
        ...
        if self.pos_snapshots.rest_shape == 'first':
            ...
            self.comps_nomean = self.comps - np.mean(self.pos_snapshots.verts, axis=0)

        testSparsity("Mean-not-added " + self.basesType + " bases", self.comps_nomean
                     , 2)
        test_linear_indpendency(self.comps_nomean
                                , 3, self.numComp)

        if orthogonal:
            self.is_utmu_orthogonal()  # test U^T M U = I (K x K)

        print("... Volkwein ("+str(massWeight)+")... standerized ("+str(standarize)+
              "), orthogonalized ("+str(orthogonal)+").")
    ...
    def store_components_to_files(self, output_bases_dir, start, end, step, fileType):
        ...
        for k in range(start, end + 1, step):
            ...
            store_components(basesnoMeanFile, numframes, k, numverts, 3, self.comps_nomean
            [:k, :, :], fileType)
        print('done.')
    ...
    def store_animations(self, output_bases_dir):
        ...
        if self.output_animation_file:
            with h5py.File(output_animation, 'w') as wh5:
                # wh5['verts'] = np.tensordot(self.weigs, self.comps, (1, 0))
